...
export class ProductionOptimizer {
  ...
  private removeDevFeatures(): void {
    if (this.isProduction) {
      ...
      // Disable development-only event listeners
      window.removeEventListener('keydown', this.handleDevKeyboard);
    }
  }

  private enableProductionErrorHandling(): void {
    if (this.isProduction) {
  ...
  private enableDevTools(): void {
    if (!this.isProduction) {
      // Enable development keyboard shortcuts
      window.addEventListener('keydown', this.handleDevKeyboard);
      
      // Add development UI indicators
      document.body.classList.add('development-mode');
    }
  }

  private handleDevKeyboard = (event: KeyboardEvent) => {
