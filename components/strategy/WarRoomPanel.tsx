                    ...
                    <Reorder.Group axis="y" values={queuedPlayers} onReorder={setQueuedPlayers} className="space-y-1 sm:px-4 md:px-6 lg:px-8">
                        {queuedPlayers.map((player: any) => (
                            <Reorder.Item 
                                key={player.id} 
                                value={player}
                                className="bg-white/5 rounded-md flex justify-between items-center group cursor-grab active:cursor-grabbing sm:px-4 md:px-6 lg:px-8"
                            >
                                <div className="flex items-center gap-2 overflow-hidden p-2 sm:px-4 md:px-6 lg:px-8">
                                     <DragHandleIcon className="h-5 w-5 text-gray-500 flex-shrink-0 sm:px-4 md:px-6 lg:px-8" />
                                    <div className="overflow-hidden sm:px-4 md:px-6 lg:px-8">
                                        <p className="font-bold text-sm truncate sm:px-4 md:px-6 lg:px-8">{player.name}</p>
                                        <p className="text-xs text-gray-400 sm:px-4 md:px-6 lg:px-8">{player.position} - {player.team}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2 p-2 sm:px-4 md:px-6 lg:px-8">
                                    <span className="font-bold text-md text-cyan-400 sm:px-4 md:px-6 lg:px-8">{player.rank}</span>
                                    <button onClick={() => handleRemove(player.id)} className="text-gray-500 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 sm:px-4 md:px-6 lg:px-8" aria-label={`Remove ${player.name} from queue`}>
...
export default React.memo(WarRoomPanelWithErrorBoundary);
