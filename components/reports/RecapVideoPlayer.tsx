...
const TitleScene: React.FC<{
  const [isLoading, setIsLoading] = React.useState(false); scene: RecapVideoScene }> = ({ scene }) => (
    ...
    >
        <h1 className="font-display text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-green-300 via-cyan-300 to-indigo-400 sm:px-4 md:px-6 lg:px-8">
            {scene.title}
        </h1>
        <p className="mt-4 text-lg text-gray-300 sm:px-4 md:px-6 lg:px-8">{scene.narration}</p>
    </motion.div>
);
...
const MatchupScene: React.FC<{ scene: RecapVideoScene, league: League }> = ({ scene, league }) => {
    ...
    return (
        ...
        >
            ...
            <motion.p {...{ initial: { y: 20, opacity: 0 }, animate: { y: 0, opacity: 1 }, transition: { delay: 0.8 } }} className="mt-8 text-lg text-gray-300 text-center italic sm:px-4 md:px-6 lg:px-8">
                "{scene.narration}"
            </motion.p>
        </motion.div>
    )
...
const TopPerformerScene: React.FC<{ scene: RecapVideoScene, league: League }> = ({ scene, league }) => {
    ...
    return (
        ...
        >
            ...
            <motion.p {...{ initial: { y: 20, opacity: 0 }, animate: { y: 0, opacity: 1 }, transition: { delay: 0.8 } }} className="mt-4 text-lg text-gray-300 text-center italic sm:px-4 md:px-6 lg:px-8">
                "{scene.narration}"
            </motion.p>
        </motion.div>
    )
...
const OutroScene: React.FC<{ scene: RecapVideoScene }> = ({ scene }) => (
    ...
    >
        <LazyImage 
          ...
          loading="lazy"
        />
        <p className="text-lg text-gray-300 sm:px-4 md:px-6 lg:px-8">{scene.narration}</p>
    </motion.div>
);
    ...
    return (
        <div className="w-full h-full flex flex-col relative text-white sm:px-4 md:px-6 lg:px-8">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent sm:px-4 md:px-6 lg:px-8">
                <div className="w-full bg-white/20 h-1.5 rounded-full sm:px-4 md:px-6 lg:px-8">
                    <motion.div 
                ...
                <div className="flex items-center justify-between mt-2 sm:px-4 md:px-6 lg:px-8">
                    <button onClick={handlePlayPause} className="p-2 rounded-full text-white bg-white/10 hover:bg-white/20 sm:px-4 md:px-6 lg:px-8" aria-label="Action button">
                        {isPlaying ? <PauseIcon /> : <PlayIcon />}
                    </button>
                    <span className="text-xs font-mono sm:px-4 md:px-6 lg:px-8">{currentIndex + 1} / {script.length}</span>
                </div>
            </div>
        </div>
    );
...
export default React.memo(RecapVideoPlayerWithErrorBoundary);
