...
const AwardCard: React.FC<{ award: DraftRecapData['awards'][0] }> = ({ award }: any) => {
    ...
    return (
        <div className="bg-white/5 p-3 sm:p-4 rounded-lg flex items-start gap-3 sm:gap-4">
            <div className="text-cyan-400 mt-1 flex-shrink-0 sm:px-4 md:px-6 lg:px-8">{getIcon(award.awardTitle)}</div>
            <div className="min-w-0 flex-1 sm:px-4 md:px-6 lg:px-8">
                ...
                <p className="text-xs sm:text-sm text-gray-300">
                    <span className="font-semibold text-yellow-300 sm:px-4 md:px-6 lg:px-8">{award.teamName}</span> - {award.playerName}
                </p>
                <p className="text-xs text-gray-400 mt-1 italic break-words sm:px-4 md:px-6 lg:px-8">"{award.rationale}"</p>
            </div>
        </div>
    );
};
...
export default React.memo(DraftRecapWithErrorBoundary);
