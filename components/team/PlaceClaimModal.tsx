        ...
        <Modal isOpen={true} onClose={onClose}>
            ...
            >
                <header className="p-6 border-b border-[var(--panel-border)] text-center sm:px-4 md:px-6 lg:px-8">
                    <h2 className="text-2xl font-bold font-display sm:px-4 md:px-6 lg:px-8">Place Waiver Claim</h2>
                    <p className="text-lg text-cyan-300 sm:px-4 md:px-6 lg:px-8">{playerToAdd.name} ({playerToAdd.position})</p>
                </header>
                        ...
                        <div className="p-4 border-t border-b border-[var(--panel-border)] sm:px-4 md:px-6 lg:px-8">
                             ...
                             {isAnalyzing ? (
                                ...
                                <div className="text-center sm:px-4 md:px-6 lg:px-8">
                                    <p className="text-xs text-gray-300 italic sm:px-4 md:px-6 lg:px-8">"{advice.summary}"</p>
                                    <p className="font-bold text-base mt-1 sm:px-4 md:px-6 lg:px-8">Suggested Bid: <span className="text-yellow-300 sm:px-4 md:px-6 lg:px-8">${advice.suggestedBid}</span></p>
                                </div>
                             )}
                        </div>
                     )}
                ...
                <footer className="p-6 flex justify-between items-center gap-4 border-t border-[var(--panel-border)] sm:px-4 md:px-6 lg:px-8">
                    ...
                    <div className="flex gap-2 sm:px-4 md:px-6 lg:px-8">
                        ...
                        <button onClick={handleSubmitClaim} className="mobile-touch-target px-6 py-3 bg-gradient-to-r from-green-500 to-cyan-500 text-white font-bold rounded-lg sm:px-4 md:px-6 lg:px-8" aria-label="Action button">Submit Claim</button>
                    </div>
                </footer>
            </motion.div>
        </Modal>
...
export default React.memo(PlaceClaimModalWithErrorBoundary);
