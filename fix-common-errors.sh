#!/bin/bash

echo "Starting batch ESLint fix process..."

# Fix React unescaped entities in all files
echo "Fixing React unescaped entities..."
find . -name "*.tsx" -not -path "./node_modules/*" -not -path "./dist/*" -exec sed -i \
  -e "s/>\([^<]*\)don't\([^<]*\)</>\1don\&apos;t\2</g" \
  -e "s/>\([^<]*\)can't\([^<]*\)</>\1can\&apos;t\2</g" \
  -e "s/>\([^<]*\)won't\([^<]*\)</>\1won\&apos;t\2</g" \
  -e "s/>\([^<]*\)isn't\([^<]*\)</>\1isn\&apos;t\2</g" \
  -e "s/>\([^<]*\)aren't\([^<]*\)</>\1aren\&apos;t\2</g" \
  -e "s/>\([^<]*\)doesn't\([^<]*\)</>\1doesn\&apos;t\2</g" \
  -e "s/>\([^<]*\)it's\([^<]*\)</>\1it\&apos;s\2</g" \
  -e "s/>\([^<]*\)you're\([^<]*\)</>\1you\&apos;re\2</g" \
  -e "s/>\([^<]*\)we're\([^<]*\)</>\1we\&apos;re\2</g" \
  -e "s/>\([^<]*\)they're\([^<]*\)</>\1they\&apos;re\2</g" \
  -e "s/>\([^<]*\)I'll\([^<]*\)</>\1I\&apos;ll\2</g" \
  -e "s/>\([^<]*\)you'll\([^<]*\)</>\1you\&apos;ll\2</g" \
  -e "s/>\([^<]*\)we'll\([^<]*\)</>\1we\&apos;ll\2</g" \
  -e "s/>\([^<]*\)let's\([^<]*\)</>\1let\&apos;s\2</g" \
  {} \;

echo "Removing console.log statements..."
find . -name "*.tsx" -not -path "./node_modules/*" -not -path "./dist/*" -exec sed -i \
  -e '/console\.log/d' \
  -e '/console\.error/d' \
  -e '/console\.warn/d' \
  {} \;

echo "Batch fixes completed!"
